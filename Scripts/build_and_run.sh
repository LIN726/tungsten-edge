...
FINAL_BUNDLE="$DERIVED_DATA_DIR/Build/Products/Debug/$DISPLAY_NAME.app"
APP_EXECUTABLE="$FINAL_BUNDLE/Contents/MacOS/$APP_NAME"
...
sign_app() {
  ...
  local framework="$FINAL_BUNDLE/Contents/Frameworks/Sparkle.framework"
  if [[ -d "$framework" ]]; then
  ...
  /usr/bin/codesign --force --sign "$identity" "$FINAL_BUNDLE" >>/tmp/macos-dock-cc-v2-codesign.log 2>&1
}

run_cli() {
  xcodebuild -project "$PROJECT_PATH" -scheme "$CLI_NAME" -configuration Debug -derivedDataPath "$DERIVED_DATA_DIR" CODE_SIGN_IDENTITY="-" build >/tmp/macos-dock-cc-v2-build.log 2>&1
...
wait_for_exit() {
  ...
  for attempt in $(seq 1 25); do
    if ! pgrep -x "$APP_NAME" >/dev/null 2>&1 && ! pgrep -x "$DISPLAY_NAME" >/dev/null 2>&1; then
      return 0
    fi
    sleep 0.2
  done
  ...
  pkill -9 -x "$DISPLAY_NAME" >/dev/null 2>&1 || true
  sleep 0.3
}
...
open_app() {
  ...
  if [[ -d "/Applications/$DISPLAY_NAME.app" ]]; then
    ...
    ditto "$FINAL_BUNDLE" "/Applications/$DISPLAY_NAME.app"
  fi
  /usr/bin/open "$FINAL_BUNDLE"
}
...
case "$MODE" in
  run|install)
    ...
    if [[ "$MODE" == "install" ]]; then
      wait_for_exit
      echo "==> 安装到 /Applications/$DISPLAY_NAME.app…"
      rm -rf "/Applications/$DISPLAY_NAME.app"
      ditto "$FINAL_BUNDLE" "/Applications/$DISPLAY_NAME.app"
      /usr/bin/open "/Applications/$DISPLAY_NAME.app"
    else
      open_app
    fi
    ;;
  ...
  --logs|logs)
    ...
    sign_app
    open_app
    /usr/bin/log stream --info --style compact --predicate "process == \"$APP_NAME\" || process == \"$DISPLAY_NAME\""
    ;;
  --telemetry|telemetry)
    pkill -x "$APP_NAME" >/dev/null 2>&1 || true
    pkill -x "$DISPLAY_NAME" >/dev/null 2>&1 || true
